...
from connors_python_menu import ConnorsPythonMenu, AsyncConnorsPythonMenu, APIResponseValidationError
from connors_python_menu._types import Omit
from connors_python_menu._models import BaseModel, FinalRequestOptions
from connors_python_menu._constants import RAW_RESPONSE_HEADER
from connors_python_menu._exceptions import (
    APIStatusError,
    APITimeoutError,
    ConnorsPythonMenuError,
    APIResponseValidationError,
)
from connors_python_menu._base_client import (
    DEFAULT_TIMEOUT,
...
def _get_open_connections(client: ConnorsPythonMenu | AsyncConnorsPythonMenu) -> int:
...
class TestConnorsPythonMenu:
    client = ConnorsPythonMenu(base_url=base_url, api_key=api_key, _strict_response_validation=True)
            ...
            for frame in diff.traceback:
                if any(
                    ...
                    for fragment in [
                        ...
                        "connors_python_menu/_legacy_response.py",
                        "connors_python_menu/_response.py",
                        # pydantic.BaseModel.model_dump || pydantic.BaseModel.dict leak memory for some reason.
                        "connors_python_menu/_compat.py",
    ...
    def test_validate_headers(self) -> None:
        ...
        with pytest.raises(ConnorsPythonMenuError):
            with update_env(**{"PETSTORE_API_KEY": Omit()}):
                client2 = ConnorsPythonMenu(base_url=base_url, api_key=None, _strict_response_validation=True)
            _ = client2
    ...
    def test_multipart_repeating_array(self, client: ConnorsPythonMenu) -> None:
        request = client._build_request(
    ...
    def test_base_url_env(self) -> None:
        with update_env(CONNORS_PYTHON_MENU_BASE_URL="http://localhost:5000/from/env"):
            client = ConnorsPythonMenu(api_key=api_key, _strict_response_validation=True)
    ...
    def test_base_url_trailing_slash(self, client: ConnorsPythonMenu) -> None:
        request = client._build_request(
            FinalRequestOptions(
                method="post",
                url="/foo",
    ...
    def test_base_url_no_trailing_slash(self, client: ConnorsPythonMenu) -> None:
        request = client._build_request(
            FinalRequestOptions(
                method="post",
                url="/foo",
    ...
    def test_absolute_request_url(self, client: ConnorsPythonMenu) -> None:
        request = client._build_request(
            FinalRequestOptions(
                method="post",
    ...
    def test_received_text_for_expected_json(self, respx_mock: MockRouter) -> None:
        ...
        strict_client = ConnorsPythonMenu(base_url=base_url, api_key=api_key, _strict_response_validation=True)

        with pytest.raises(APIResponseValidationError):
            strict_client.get("/foo", cast_to=Model)

        client = ConnorsPythonMenu(base_url=base_url, api_key=api_key, _strict_response_validation=False)
    ...
    @mock.patch("connors_python_menu._base_client.BaseClient._calculate_retry_timeout", _low_retry_timeout)
    @pytest.mark.respx(base_url=base_url)
    ...
    def test_omit_retry_count_header(
        self, client: ConnorsPythonMenu, failures_before_success: int, respx_mock: MockRouter
    ) -> None:
        client = client.with_options(max_retries=4)
...
class TestAsyncConnorsPythonMenu:
    client = AsyncConnorsPythonMenu(base_url=base_url, api_key=api_key, _strict_response_validation=True)
    ...
    def test_default_headers_option(self) -> None:
        ...
        client2 = AsyncConnorsPythonMenu(
            base_url=base_url,
            api_key=api_key,
            _strict_response_validation=True,
            default_headers={
    ...
    def test_validate_headers(self) -> None:
        ...
        with pytest.raises(ConnorsPythonMenuError):
            with update_env(**{"PETSTORE_API_KEY": Omit()}):
                client2 = AsyncConnorsPythonMenu(base_url=base_url, api_key=None, _strict_response_validation=True)
            _ = client2
    ...
    def test_multipart_repeating_array(self, async_client: AsyncConnorsPythonMenu) -> None:
    ...
    def test_base_url_env(self) -> None:
        with update_env(CONNORS_PYTHON_MENU_BASE_URL="http://localhost:5000/from/env"):
            client = AsyncConnorsPythonMenu(api_key=api_key, _strict_response_validation=True)
            assert client.base_url == "http://localhost:5000/from/env/"
    ...
    def test_base_url_trailing_slash(self, client: AsyncConnorsPythonMenu) -> None:
        request = client._build_request(
            FinalRequestOptions(
                method="post",
                url="/foo",
    ...
    def test_base_url_no_trailing_slash(self, client: AsyncConnorsPythonMenu) -> None:
        request = client._build_request(
            FinalRequestOptions(
                method="post",
                url="/foo",
    ...
    def test_absolute_request_url(self, client: AsyncConnorsPythonMenu) -> None:
        request = client._build_request(
            FinalRequestOptions(
                method="post",
                url="https://myapi.com/foo",
    ...
    async def test_received_text_for_expected_json(self, respx_mock: MockRouter) -> None:
        ...
        strict_client = AsyncConnorsPythonMenu(base_url=base_url, api_key=api_key, _strict_response_validation=True)

        with pytest.raises(APIResponseValidationError):
            await strict_client.get("/foo", cast_to=Model)

        client = AsyncConnorsPythonMenu(base_url=base_url, api_key=api_key, _strict_response_validation=False)
    ...
    async def test_retries_taken(
        self,
        async_client: AsyncConnorsPythonMenu,
        failures_before_success: int,
        failure_mode: Literal["status", "exception"],
        respx_mock: MockRouter,
    ) -> None:
    ...
    async def test_omit_retry_count_header(
        self, async_client: AsyncConnorsPythonMenu, failures_before_success: int, respx_mock: MockRouter
    ) -> None:
        client = async_client.with_options(max_retries=4)
    ...
    def test_get_platform(self) -> None:
        ...
        from connors_python_menu._utils import asyncify
        from connors_python_menu._base_client import get_platform
